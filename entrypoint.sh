...
then
  ...
  echo ""
  echo 'COMMANDS'
  echo '  authserver [--ip-addr <ip-address>]'
  ...
  echo '      --ip-addr flag'
  echo ""
  echo '      <ip-address> is the ip address of the computer you are running the warcraft'
  echo '      server on. Specify a public ip if you want others outside of your local'
  echo '      network to connect to your server. This must be the same as the'
  echo '      worldserver ip address.'
  echo ""
  echo '  data'
  ...
  echo ""
  echo '  update-ip <ip-address>'
  echo '      Update the ip address of the worldserver and authserver to'
  echo '      <ip-address>. Specify a public ip address if you want others outside'
  echo '      of your local network to connect to your server.'
  echo ""
  echo '  worldserver [--ip-addr <ip-address> --data-dir <path/to/maps/dir>]'
  ...
  echo '      you must specify both --ip-addr AND --data-dir.'
  echo ""
  echo '      <ip-address> is the ip address of the computer you are running the warcraft'
  echo '      server on. Specify a public ip if you want others outside of your local'
  echo '      network to connect to your server. This must be the same as the'
  echo '      authserver ip address.'
  echo ""
  echo '      <path/to/maps/dir> is the path to the folder containing the maps, mmaps, vmaps,'
